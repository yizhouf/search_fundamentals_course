...
def add_click_priors(query_obj, user_query, priors_gb):
    try:
        ...
        if prior_clicks_for_query is not None and len(prior_clicks_for_query) > 0:
            click_prior = " " # for quick turn-off of the prior feature???
            #### W2, L1, S1
            # Create a string object of SKUs and weights that will boost documents matching the SKU
            # print("TODO: Implement me")
            if click_prior != "":
                counts = prior_clicks_for_query['sku'].value_counts(normalize = True)
                for ii in range(len(counts)): #not sure how to do this properly as instructed, looping through using term as a blunt solution. 
                    click_prior_query_obj = {
                        "term": {
                            "sku": {
                                "value": counts.index[ii],
                                "boost": counts.iloc[ii]*100 #debuged using query "ipd" and identify this 100 ratio value to boost the pink ipad case up (id: 1218321915419)
                            }
                        }
                    }
                    query_obj["query"]["function_score"]["query"]["bool"]["should"].append(click_prior_query_obj)
            
            # print(prior_clicks_for_query['sku'].value_counts(normalize = True))
            # if click_prior != "":
